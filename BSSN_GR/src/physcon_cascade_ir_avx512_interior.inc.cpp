// 8-wide AVX-512 driver for physconeqs_cascade_ir_avx.cpp; the AVX2 twin runs
// the same body 4 lanes wide. psi4's tetrad needs the point coordinates: y and
        ...
        for (unsigned int k = PW; k < nz - PW; k++) {
            ...
            for (unsigned int j = PW; j < ny - PW; j++) {
                ...
                auto run_batch = [&](unsigned int i_start) {
                    ...
                    alignas(64) double x_arr[PHYSCON_LANES];
                    for (int l = 0; l < PHYSCON_LANES; l++)
                        x_arr[l] = pmin[0] + (i_start + l) * hx;

                    const VEC x = _mm512_load_pd(x_arr);
                    const VEC y = VSET(yc);
                    const VEC z = VSET(zc);
