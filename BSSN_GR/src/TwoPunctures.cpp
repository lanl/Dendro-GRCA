...
void TwoPunctures(const double xx1, const double yy1, const double zz1,
    ...
    *mm = TPID::par_m_minus;
    ...
    if (brownsville_lapse) {
        /*if(!rank)
         printf( "Setting initial lapse to a Brownsville-style profile with exp
         %f.", TPID::initial_lapse_psi_exponent);*/
    }
    ...
    switch (gsm) {
        case GSM_Taylor_expansion:
            ...
            break;
        case GSM_evaluation:
            U = PunctIntPolAtArbitPositionFast(0, nvar, n1, n2, n3, cf_v, xx,
                                               yy, zz);
            break;
        default:
            assert(0);
    ...
    if (r_plus < TPID::TP_Tiny) r_plus = TPID::TP_Tiny;
    if (r_minus < TPID::TP_Tiny) r_minus = TPID::TP_Tiny;
    ...
    if ((conformal_state > 0) || (pmn_lapse) || (brownsville_lapse)) {
        ...
        if (rp < TPID::TP_Tiny) rp = TPID::TP_Tiny;
