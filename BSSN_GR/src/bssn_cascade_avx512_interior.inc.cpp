// bssn_cascade_avx512_interior.inc.cpp -- drop-in replacement for the interior
// RHS triple loop using the AVX-512-batched cascade (8 lanes per SIMD op).
//
// Included in BSSN_GR/src/rhs.cpp under #ifdef BSSN_USE_CASCADE_AVX512.
...
// bssn_bcs() as usual after this loop. Assumes BSSN_USE_CASCADE_AVX512 is
// compatible with the standard gauge (no Rochester/SSL/CAHD).
//
// This is the 8-wide twin of bssn_cascade_avx_interior.inc.cpp; the only
// differences are the SIMD batch width (8 vs 4), the eta pack, and the
// generated body it pulls in (bssneqs_cascade_avx512.cpp). For blocks whose
// interior x-extent is < 8, rhs.cpp falls back to the 4-wide AVX2 interior.

// <immintrin.h> and <cmath> are included at top of rhs.cpp under
// #ifdef BSSN_USE_CASCADE_AVX512.
{
        ...
        for (unsigned int k = PW; k < nz - PW; k++) {
            ...
            for (unsigned int j = PW; j < ny - PW; j++) {
                ...
                auto run_batch = [&](unsigned int i_start) {
                    ...
                    #include "bssneqs_cascade_avx512.cpp"
                };
