...
void computeRHS(double **unzipVarsRHS, const double **uZipVars,
                ...
                const double **uZipConstVars, unsigned int device) {
    cuda::profile::t_overall.start();

    if (numStreams == 0) {
        std::cout << "[Error]: " << __func__ << " numStreams " << numStreams
                  << " should at least be 1 (synchronous transfer) "
                  << std::endl;
        return;
    }
    ...
    const unsigned int UNZIP_DOF_SZ =
        dendroBlockList[numBlocks - 1].getOffset() +
        dendroBlockList[numBlocks - 1].getAlignedBlockSz();

    // get GPU information.
    //  assumes the if there are multiple gpus per node all have the same
    //  specification.
    ...
    for (unsigned int blk = 0; blk < numBlocks; blk++) {
        ...
        if (bflag == 0) {
            ...
            ptmin[0] =
                GRIDX_TO_X(dendroBlockList[blk].getBlockNode().minX()) - 3 * dx;
            ptmin[1] =
                GRIDY_TO_Y(dendroBlockList[blk].getBlockNode().minY()) - 3 * dy;
            ptmin[2] =
                GRIDZ_TO_Z(dendroBlockList[blk].getBlockNode().minZ()) - 3 * dz;

            ptmax[0] =
                GRIDX_TO_X(dendroBlockList[blk].getBlockNode().maxX()) + 3 * dx;
            ptmax[1] =
                GRIDY_TO_Y(dendroBlockList[blk].getBlockNode().maxY()) + 3 * dy;
            ptmax[2] =
                GRIDZ_TO_Z(dendroBlockList[blk].getBlockNode().maxZ()) + 3 * dz;

            cudaBlkList[c_in] = cuda::_Block(
                (const double *)ptmin, (const double *)ptmax, offset, bflag,
    ...
    if (numStreams == 1) {  // sync case
        // computes 1D grid block
        dim3 gridDim;
        unsigned int *blockMap = NULL;
        cuda::computeDendroBlockToGPUMap(blkListGPU, NUM_GPU_DENDRO_BLOCKS,
                                         blockMap, gridDim);

        cuda::profile::t_H2D_Comm.start();
        const unsigned int NUM_GPU_BLOCKS =
            ((gridDim.x) * (gridDim.y) * (gridDim.z));

        // send blocks to the gpu
        cuda::__DENDRO_BLOCK_LIST =
            cuda::copyArrayToDevice(cudaBlkList, NUM_GPU_DENDRO_BLOCKS);
        cuda::__DENDRO_NUM_BLOCKS =
            cuda::copyValueToDevice(&NUM_GPU_DENDRO_BLOCKS);

        cuda::__NUM_GPU_BLOCKS = cuda::copyValueToDevice(&NUM_GPU_BLOCKS);
        cuda::__GPU_BLOCK_MAP =
            cuda::copyArrayToDevice(blockMap, 2 * NUM_GPU_BLOCKS);

        cuda::__BSSN_NUM_VARS = cuda::copyValueToDevice(&BSSN_NUM_VARS);
        cuda::__BSSN_CONSTRAINT_NUM_VARS =
            cuda::copyValueToDevice(&BSSN_CONSTRAINT_NUM_VARS);

        cuda::__GPU_BLOCK_SHARED_MEM_UTIL =
            cuda::copyValueToDevice(&GPU_BLOCK_SHARED_MEM_UTIL);

        // allocate memory for unzip vectors
        cuda::__UNZIP_INPUT = cuda::alloc2DCudaArray<double>(
            uZipVars, BSSN_NUM_VARS, UNZIP_DOF_SZ);
        cuda::__UNZIP_OUTPUT =
            cuda::alloc2DCudaArray<double>(BSSN_NUM_VARS, UNZIP_DOF_SZ);
        ...
        cuda::__computeBSSNRHS<<<gridDim, blockDim>>>(
            cuda::__UNZIP_OUTPUT, (const double **)cuda::__UNZIP_INPUT,
            cuda::__BSSN_DERIV_WORKSPACE, cuda::__DENDRO_BLOCK_LIST,
            cuda::__GPU_BLOCK_MAP, cuda::__BSSN_COMPUTE_PARMS,
            cuda::__CUDA_DEVICE_PROPERTIES, 0);
        ...
        cuda::copy2DArrayToHost(unzipVarsRHS,
                                (const double **)cuda::__UNZIP_OUTPUT,
                                BSSN_NUM_VARS, UNZIP_DOF_SZ);
        ...
        for (unsigned int blk = 0; blk < NUM_CPU_DENDRO_BLOCKS; blk++) {
            ...
            ptmin[0] =
                GRIDX_TO_X(blkListCPU[blk].getBlockNode().minX()) - 3 * dx;
            ptmin[1] =
                GRIDY_TO_Y(blkListCPU[blk].getBlockNode().minY()) - 3 * dy;
            ptmin[2] =
                GRIDZ_TO_Z(blkListCPU[blk].getBlockNode().minZ()) - 3 * dz;

            ptmax[0] =
                GRIDX_TO_X(blkListCPU[blk].getBlockNode().maxX()) + 3 * dx;
            ptmax[1] =
                GRIDY_TO_Y(blkListCPU[blk].getBlockNode().maxY()) + 3 * dy;
            ptmax[2] =
                GRIDZ_TO_Z(blkListCPU[blk].getBlockNode().maxZ()) + 3 * dz;

            bssnrhs(unzipVarsRHS, (const double **)uZipVars, offset, ptmin,
                    ptmax, sz, bflag, curr_time,
        ...
        // send counts and offset for asynctransfer, all the counts and offsets
        // are based on the how we transfer the data blocks to the gpu.
        ...
        for (unsigned int i = 0; i < numStreams; i++) {
            sendBlockCount[i] =
                (((i + 1) * NUM_GPU_DENDRO_BLOCKS) / numStreams) -
                (((i)*NUM_GPU_DENDRO_BLOCKS) / numStreams);
            sendUnzipCount[i] = 0;
            for (unsigned int blk = (((i)*NUM_GPU_DENDRO_BLOCKS) / numStreams);
                 blk < (((i + 1) * NUM_GPU_DENDRO_BLOCKS) / numStreams); blk++)
        ...
        for (unsigned int i = 0; i < numStreams; i++) {
            cuda::computeDendroBlockToGPUMap(blkListGPU + sendBlockOffset[i],
                                             sendBlockCount[i], blockMap[i],
                                             gridDim[i]);
        ...
        const unsigned int NUM_GPU_BLOCKS =
            gridBlockOffset[numStreams - 1] + gridBlockCount[numStreams - 1];
        ...
        cuda::__DENDRO_BLOCK_LIST =
            cuda::alloc1DCudaArray<_Block>(NUM_GPU_DENDRO_BLOCKS);
        cuda::__GPU_BLOCK_MAP =
            cuda::alloc1DCudaArray<unsigned int>(2 * NUM_GPU_BLOCKS);
        cuda::__UNZIP_INPUT = cuda::alloc2DCudaArray<double>(
            unZipInputHost, BSSN_NUM_VARS, UNZIP_DOF_SZ);
        cuda::__UNZIP_OUTPUT = cuda::alloc2DCudaArray<double>(
            unZipOutputHost, BSSN_NUM_VARS, UNZIP_DOF_SZ);
        ...
        for (unsigned int i = 0; i < numStreams; i++) {
            for (unsigned int var = 0; var < BSSN_NUM_VARS; var++)
                cuda::copyArrayToDeviceAsync(
                    uZipVars[var] + sendUnzipOffset[i],
                    unZipInputHost[var] + sendUnzipOffset[i], sendUnzipCount[i],
                    streams[i]);

            cuda::copyArrayToDeviceAsync(
                cudaBlkList + sendBlockOffset[i],
                cuda::__DENDRO_BLOCK_LIST + sendBlockOffset[i],
                sendBlockCount[i], streams[i]);
            cuda::copyArrayToDeviceAsync(
                blockMap[i], cuda::__GPU_BLOCK_MAP + 2 * gridBlockOffset[i],
                2 * gridBlockCount[i], streams[i]);
        }

        for (unsigned int i = 0; i < numStreams; i++) {
            cuda::__computeBSSNRHS<<<gridDim[i], blockDim, 0, streams[i]>>>(
                cuda::__UNZIP_OUTPUT, (const double **)cuda::__UNZIP_INPUT,
                cuda::__BSSN_DERIV_WORKSPACE, cuda::__DENDRO_BLOCK_LIST,
                cuda::__GPU_BLOCK_MAP + 2 * gridBlockOffset[i],
                cuda::__BSSN_COMPUTE_PARMS, cuda::__CUDA_DEVICE_PROPERTIES, i);
            CUDA_CHECK_ERROR();
        }
        ...
        for (unsigned int i = 0; i < numStreams; i++) {
            // D2H
            for (unsigned int var = 0; var < BSSN_NUM_VARS; var++)
                cuda::copyArrayToHostAsync(
                    unzipVarsRHS[var] + sendUnzipOffset[i],
                    unZipOutputHost[var] + sendUnzipOffset[i],
                    sendUnzipCount[i], streams[i]);
        }
        ...
        for (unsigned int blk = 0; blk < NUM_CPU_DENDRO_BLOCKS; blk++) {
            ...
            bssnrhs(unZipRHS_CPU, (const double **)uZipVars, offset, ptmin,
                    ptmax, sz, bflag, curr_time,
                    (const double **)uZipConstVars);
        }
        ...
        for (unsigned int blk = 0; blk < NUM_CPU_DENDRO_BLOCKS; blk++) {
            ...
            for (unsigned int var = 0; var < BSSN_NUM_VARS; var++)
                for (unsigned int node = 0; node < (sz[0] * sz[1] * sz[2]);
                     node++)
                    unzipVarsRHS[var][offset + node] =
                        unZipRHS_CPU[var][offset + node];
        }
        ...
        for (unsigned int i = 0; i < numStreams; i++) delete[] blockMap[i];
