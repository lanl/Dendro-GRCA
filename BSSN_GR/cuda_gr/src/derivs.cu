...
__device__ void _RSWS_deriv42_x_1D(double* const Dxu, const double* const u,
                                   const double dx, const unsigned int* ijk_lm,
                                   const unsigned int* sz,
                                   const unsigned int* tile_sz, unsigned int pw,
                                   unsigned bflag) {
    ...
    for (unsigned int i = (i_b - ijk_lm[0]) + threadIdx.x;
         i < (i_e - ijk_lm[0]); i += blockDim.x) {
        Dxu[i] =
            (u[i - 2] - 8.0 * u[i - 1] + 8.0 * u[i + 1] - u[i + 2]) * idx_by_12;
    }
}
...
__device__ void _RSWS_deriv42_x(double* const Dxu, const double* const u,
                                const double dx, const unsigned int* ijk_lm,
                                const unsigned int* sz,
                                const unsigned int* tile_sz, unsigned int pw,
                                unsigned bflag) {
    ...
    const unsigned int k =
        (k_b - ijk_lm[4]) + threadIdx.y + threadIdx.z * blockDim.y;
    ...
    // printf("dx threadid (%d,%d,%d) loop begin: (%d,%d,%d) loop end:
    // (%d,%d,%d)  tile begin: (%d,%d,%d) tile end: (%d,%d,%d) \n",
    ...
    for (unsigned int i = (i_b - ijk_lm[0]); i <= (i_e - ijk_lm[0]); ++i, ++pp)
        Dxu[pp] = (u[pp - 2] - 8.0 * u[pp - 1] + 8.0 * u[pp + 1] - u[pp + 2]) *
                  idx_by_12;
...
__device__ void _RSWS_deriv42_y(double* const Dyu, const double* const u,
                                const double dy, const unsigned int* ijk_lm,
                                const unsigned int* sz,
                                const unsigned int* tile_sz, unsigned int pw,
                                unsigned bflag) {
    ...
    // printf("dy threadid (%d,%d,%d) loop begin: (%d,%d,%d) loop end:
    // (%d,%d,%d)  tile begin: (%d,%d,%d) tile end: (%d,%d,%d) \n",
    // threadIdx.x,threadIdx.y,threadIdx.z,ix_b,jy_b,kz_b,ix_e,jy_e,kz_e,ijk_lm[0],ijk_lm[2],ijk_lm[4],ijk_lm[1],ijk_lm[3],ijk_lm[5]);

    unsigned int pp        = IDX_L(i, (j_b - ijk_lm[2]), k);
    for (unsigned int j = (j_b - ijk_lm[2]); j <= (j_e - ijk_lm[2]);
         j += 1, pp += tile_sz[0])
        Dyu[pp] = (u[pp - 2 * tile_sz[0]] - 8.0 * u[pp - tile_sz[0]] +
                   8.0 * u[pp + tile_sz[0]] - u[pp + 2 * tile_sz[0]]) *
                  idy_by_12;
...
__device__ void _RSWS_deriv42_z(double* const Dzu, const double* const u,
                                const double dz, const unsigned int* ijk_lm,
                                const unsigned int* sz,
                                const unsigned int* tile_sz, unsigned int pw,
                                unsigned bflag) {
    const unsigned int i_b = max((int)ijk_lm[2 * 0 + 0], (int)3);
    ...
    const unsigned int j =
        (j_b - ijk_lm[2]) + threadIdx.y + threadIdx.z * blockDim.y;
    const unsigned int i = (i_b - ijk_lm[0]) + threadIdx.x;
    ...
    // printf("dz threadid (%d,%d,%d) loop begin: (%d,%d,%d) loop end:
    // (%d,%d,%d)  tile begin: (%d,%d,%d) tile end: (%d,%d,%d) \n",
    // threadIdx.x,threadIdx.y,threadIdx.z,ix_b,jy_b,kz_b,ix_e,jy_e,kz_e,ijk_lm[0],ijk_lm[2],ijk_lm[4],ijk_lm[1],ijk_lm[3],ijk_lm[5]);
    unsigned int pp        = IDX_L(i, j, (k_b - ijk_lm[4]));
    for (unsigned int k = (k_b - ijk_lm[4]); k <= (k_e - ijk_lm[4]);
         k += 1, pp += (tile_sz[0] * tile_sz[1]))
        Dzu[pp] = (u[pp - 2 * tile_sz[0] * tile_sz[1]] -
                   8.0 * u[pp - tile_sz[0] * tile_sz[1]] +
                   8.0 * u[pp + tile_sz[0] * tile_sz[1]] -
                   u[pp + 2 * tile_sz[0] * tile_sz[1]]) *
                  idz_by_12;
...
__device__ void _RSWS_deriv42_xx(double* const DxDxu, const double* const u,
                                 const double dx, const unsigned int* ijk_lm,
                                 const unsigned int* sz,
                                 const unsigned int* tile_sz, unsigned int pw,
                                 unsigned bflag) {
    ...
    unsigned int pp             = IDX_L((i_b - ijk_lm[0]), j, k);
    for (unsigned int i = (i_b - ijk_lm[0]); i <= (i_e - ijk_lm[0]);
         i += 1, ++pp) {
...
__device__ void _RSWS_deriv42_yy(double* const DyDyu, const double* const u,
                                 const double dy, const unsigned int* ijk_lm,
                                 const unsigned int* sz,
                                 const unsigned int* tile_sz, unsigned int pw,
                                 unsigned bflag) {
    ...
    for (unsigned int j = (j_b - ijk_lm[2]); j <= (j_e - ijk_lm[2]);
         j += 1, pp += tile_sz[0]) {
        DyDyu[pp] = (-u[pp - 2 * tile_sz[0]] + 16.0 * u[pp - tile_sz[0]] -
                     30.0 * u[pp] + 16.0 * u[pp + tile_sz[0]] -
                     u[pp + 2 * tile_sz[0]]) *
                    idy_sqrd_by_12;
    }
...
__device__ void _RSWS_deriv42_zz(double* const DzDzu, const double* const u,
                                 const double dz, const unsigned int* ijk_lm,
                                 const unsigned int* sz,
                                 const unsigned int* tile_sz, unsigned int pw,
                                 unsigned bflag) {
    ...
    for (unsigned int k = (k_b - ijk_lm[4]); k <= (k_e - ijk_lm[4]);
         k += 1, pp += (tile_sz[0] * tile_sz[1])) {
        DzDzu[pp] = (-u[pp - 2 * tile_sz[0] * tile_sz[1]] +
                     16.0 * u[pp - tile_sz[0] * tile_sz[1]] - 30.0 * u[pp] +
                     16.0 * u[pp + tile_sz[0] * tile_sz[1]] -
                     u[pp + 2 * tile_sz[0] * tile_sz[1]]) *
                    idz_sqrd_by_12;
    }
 ...
 *
 *----------------------------------------------------------------------*/
__device__ void _RSWS_deriv42adv_x(double* const Dxu, const double* const u,
                                   const double dx, const unsigned int* ijk_lm,
                                   const unsigned int* sz,
                                   const unsigned int* tile_sz,
                                   const bool* const betax, unsigned int pw,
                                   unsigned bflag) {
    const unsigned int i_b = ijk_lm[2 * 0 + 0] + pw;
    const unsigned int i_e = ijk_lm[2 * 0 + 1] - pw;
    ...
    for (unsigned int i = (i_b - ijk_lm[0]); i <= (i_e - ijk_lm[0]);
         i += 1, ++pp) {
        (betax[pp])
            ? Dxu[pp] = (-3.0 * u[pp - 1] - 10.0 * u[pp] + 18.0 * u[pp + 1] -
                         6.0 * u[pp + 2] + u[pp + 3]) *
                        idx_by_12
            ...
            for(unsigned int k=kz_b;k<kz_e;k++)
                for(unsigned int j=jy_b;j<jy_e;j++)
                {
                    ...
                    if (betax[IDX_L(ix_b+1, j, k)] > 0.0) {
                        Dxu[IDX_L(ix_b+1, j, k)] = (-3.0 * u[IDX_L(ix_b+1, j,
    k)]
                                            + 4.0 * u[IDX_L(ix_b+2, j, k)]
                                            - u[IDX_L(ix_b+3, j, k)]
                                            ) * idx_by_2;
                    ...
                    if (betax[IDX_L(ix_b+2, j, k)] > 0.0) {
                        Dxu[IDX_L(ix_b+2, j, k)] = (-3.0 * u[IDX_L(ix_b+1, j,
    k)]
            ...
            for(unsigned int k=kz_b;k<kz_e;k++)
                for(unsigned int j=jy_b;j<jy_e;j++)
                {
                    if (betax[IDX_L(ix_e - 3, j, k)] < 0.0) {
                        Dxu[IDX_L(ix_e - 3, j, k)] = (-3.0 * u[IDX_L(ix_e - 3,
    j, k)]
                                                  + 4.0 * u[IDX_L(ix_e - 2, j,
    k)]
                                                  - u[IDX_L(ix_e - 1, j, k)]
                                                 ) * idx_by_2;
                    } else {
                        Dxu[IDX_L(ix_e - 3, j, k)] = (-u[IDX_L(ix_e - 6, j, k)]
                                                  + 6.0 * u[IDX_L(ix_e - 5, j,
    k)]
                                                  - 18.0 * u[IDX_L(ix_e - 4, j,
    k)]
                                                  + 10.0 * u[IDX_L(ix_e - 3, j,
    k)]
                                                  + 3.0 * u[IDX_L(ix_e - 2, j,
    k)] ) * idx_by_12;
                    }
 ...
 *
 *----------------------------------------------------------------------*/
__device__ void _RSWS_deriv42adv_y(double* const Dyu, const double* const u,
                                   const double dy, const unsigned int* ijk_lm,
                                   const unsigned int* sz,
                                   const unsigned int* tile_sz,
                                   const bool* const betay, unsigned int pw,
                                   unsigned bflag) {
    const unsigned int i_b = max((int)ijk_lm[2 * 0 + 0], (int)3);
    const unsigned int i_e = min((int)ijk_lm[2 * 0 + 1], sz[0] - 3);
    ...
    for (unsigned int j = (j_b - ijk_lm[2]); j <= (j_e - ijk_lm[2]);
        ...
        (betay[pp])
            ? Dyu[pp] =
                  (-3.0 * u[pp - tile_sz[0]] - 10.0 * u[pp] +
                   18.0 * u[pp + tile_sz[0]] - 6.0 * u[pp + 2 * tile_sz[0]] +
                   u[pp + 3 * tile_sz[0]]) *
                  idy_by_12
            : Dyu[pp] =
                  (-u[pp - 3 * tile_sz[0]] + 6.0 * u[pp - 2 * tile_sz[0]] -
                   18.0 * u[pp - tile_sz[0]] + 10.0 * u[pp] +
                   3.0 * u[pp + tile_sz[0]]) *
                  idy_by_12;
    }
            ...
            for(unsigned int k=kz_b;k<kz_e;k++)
                for(unsigned int i=ix_b;i<ix_e;i++)
                {
                    ...
                    if (betay[IDX_L(i, jy_b+1, k)] > 0.0) {
                        Dyu[IDX_L(i, jy_b+1, k)] = (-3.0 * u[IDX_L(i, jy_b+1,
    k)]
                                            + 4.0 * u[IDX_L(i, jy_b+2, k)]
                                            - u[IDX_L(i, jy_b+3, k)]
                                            ) * idy_by_2;
                    ...
                    if (betay[IDX_L(i, jy_b+2, k)] > 0.0) {
                        Dyu[IDX_L(i, jy_b+2, k)] = (-3.0 * u[IDX_L(i, jy_b+1,
    k)]
            ...
            for(unsigned int k=kz_b;k<kz_e;k++)
                for(unsigned int i=ix_b;i<ix_e;i++)
                {
                    if (betay[IDX_L(i, jy_e - 3, k)] < 0.0) {
                        Dyu[IDX_L(i, jy_e - 3, k)] = (-3.0 * u[IDX_L(i, jy_e -
    3, k)]
                                                  + 4.0 * u[IDX_L(i, jy_e - 2,
    k)]
                                                  - u[IDX_L(i, jy_e - 1, k)]
                                                 ) * idy_by_2;
                    } else {
                        Dyu[IDX_L(i, jy_e - 3, k)] = (-u[IDX_L(i, jy_e - 6, k)]
                                                  + 6.0 * u[IDX_L(i, jy_e - 5,
    k)]
                                                  - 18.0 * u[IDX_L(i, jy_e - 4,
    k)]
                                                  + 10.0 * u[IDX_L(i, jy_e - 3,
    k)]
                                                  + 3.0 * u[IDX_L(i, jy_e - 2,
    k)] ) * idy_by_12;
                    }
...
__device__ void _RSWS_deriv42adv_z(double* const Dzu, const double* const u,
                                   const double dz, const unsigned int* ijk_lm,
                                   const unsigned int* sz,
                                   const unsigned int* tile_sz,
                                   const bool* const betaz, unsigned int pw,
                                   unsigned bflag) {
    const unsigned int i_b = max((int)ijk_lm[2 * 0 + 0], (int)3);
    const unsigned int i_e = min((int)ijk_lm[2 * 0 + 1], sz[0] - 3);
    ...
    for (unsigned int k = (k_b - ijk_lm[4]); k <= (k_e - ijk_lm[4]);
         k += 1, pp += (tile_sz[0] * tile_sz[1])) {
        (betaz[pp])
            ? Dzu[pp] = (-3.0 * u[pp - tile_sz[0] * tile_sz[1]] - 10.0 * u[pp] +
                         18.0 * u[pp + tile_sz[0] * tile_sz[1]] -
                         6.0 * u[pp + 2 * tile_sz[0] * tile_sz[1]] +
                         u[pp + 3 * tile_sz[0] * tile_sz[1]]) *
                        idz_by_12
            : Dzu[pp] = (-u[pp - 3 * tile_sz[0] * tile_sz[1]] +
                         6.0 * u[pp - 2 * tile_sz[0] * tile_sz[1]] -
                         18.0 * u[pp - tile_sz[0] * tile_sz[1]] + 10.0 * u[pp] +
                         3.0 * u[pp + tile_sz[0] * tile_sz[1]]) *
                        idz_by_12;
    }
            ...
            for(unsigned int j=jy_b;j<jy_e;j++)
                for(unsigned int i=ix_b;i<ix_e;i++)
                {
                    ...
                    if (betaz[IDX_L(i, j, kz_b+1)] > 0.0) {
                        Dzu[IDX_L(i, j, kz_b+1)] = (-3.0 * u[IDX_L(i, j,
    kz_b+1)]
                                            + 4.0 * u[IDX_L(i, j, kz_b+2)]
                                            - u[IDX_L(i, j, kz_b+3)]
                                            ) * idz_by_2;
                    ...
                    if (betaz[IDX_L(i, j, kz_b+2)] > 0.0) {
                        Dzu[IDX_L(i, j, kz_b+2)] = (-3.0 * u[IDX_L(i, j,
    kz_b+1)]
            ...
            for(unsigned int j=jy_b;j<jy_e;j++)
                for(unsigned int i=ix_b;i<ix_e;i++)
                {
                    ...
                    if (betaz[IDX_L(i, j, kz_e - 3)] < 0.0) {
                        Dzu[IDX_L(i, j, kz_e - 3)] = (-3.0 * u[IDX_L(i, j, kz_e
    - 3)]
                                                  + 4.0 * u[IDX_L(i, j, kz_e -
    2)]
                                                  - u[IDX_L(i, j, kz_e - 1)]
                                                 ) * idz_by_2;
                    } else {
    ...
    5)]
                                                  - 18.0 * u[IDX_L(i, j, kz_e -
    4)]
                                                  + 10.0 * u[IDX_L(i, j, kz_e -
    3)]
                                                  + 3.0 * u[IDX_L(i, j, kz_e -
    2)] ) * idz_by_12;
                    }
...
__device__ void _RSWS_ko_deriv42_x(double* const Du, const double* const u,
                                   const double dx, const unsigned int* ijk_lm,
                                   const unsigned int* sz,
                                   const unsigned int* tile_sz, unsigned int pw,
                                   unsigned bflag) {
    ...
    for (unsigned int i = (i_b - ijk_lm[0]); i <= (i_e - ijk_lm[0]);
        ...
        Du[pp] =
            pre_factor_6_dx *
            (-u[pp - 3] + 6.0 * u[pp - 2] - 15.0 * u[pp - 1] + 20.0 * u[pp] -
             15.0 * u[pp + 1] + 6.0 * u[pp + 2] - u[pp + 3]);
    }

    if (ijk_lm[0] == pw) {
        Du[IDX_L((i_b - ijk_lm[0]), j, k)] =
            pre_factor_6_dx * (-u[IDX_L((i_b - ijk_lm[0]) + 4, j, k)] +
    ...
    if (ijk_lm[1] == (sz[0] - 1 - pw)) {
        Du[IDX_L((i_e - ijk_lm[0]), j, k)] =
            pre_factor_6_dx * (-u[IDX_L((i_e - ijk_lm[0]) + 1, j, k)] +
...
__device__ void _RSWS_ko_deriv42_y(double* const Du, const double* const u,
                                   const double dy, const unsigned int* ijk_lm,
                                   const unsigned int* sz,
                                   const unsigned int* tile_sz, unsigned int pw,
                                   unsigned bflag) {
    ...
    unsigned int pp              = IDX_L(i, (j_b - ijk_lm[2]), k);
    for (unsigned int j = (j_b - ijk_lm[2]); j <= (j_e - ijk_lm[2]);
         j += 1, pp += (tile_sz[0] * tile_sz[1])) {
        Du[pp] = pre_factor_6_dy *
                 (-u[pp - 3 * tile_sz[0]] + 6.0 * u[pp - 2 * tile_sz[0]] -
                  15.0 * u[pp - tile_sz[0]] + 20.0 * u[pp] -
                  15.0 * u[pp + tile_sz[0]] + 6.0 * u[pp + 2 * tile_sz[0]] -
                  u[pp + 3 * tile_sz[0]]);
    }

    if ((ijk_lm[2]) == pw) {
        Du[IDX_L(i, (j_b - ijk_lm[2]), k)] =
            pre_factor_6_dy * (-u[IDX_L(i, (j_b - ijk_lm[2]) + 4, k)] +
    ...
    if (ijk_lm[3] == (sz[1] - 1 - pw)) {
        Du[IDX_L(i, (j_e - ijk_lm[2]), k)] =
            pre_factor_6_dy * (-u[IDX_L(i, (j_e - ijk_lm[2]) + 1, k)] +
...
__device__ void _RSWS_ko_deriv42_z(double* const Du, const double* const u,
                                   const double dz, const unsigned int* ijk_lm,
                                   const unsigned int* sz,
                                   const unsigned int* tile_sz, unsigned int pw,
                                   unsigned bflag) {
    ...
    for (unsigned int k = (k_b - ijk_lm[4]); k <= (k_e - ijk_lm[4]);
        ...
        Du[pp] = pre_factor_6_dz *
                 (-u[pp - 3 * tile_sz[0] * tile_sz[1]] +
                  6.0 * u[pp - 2 * tile_sz[0] * tile_sz[1]] -
                  15.0 * u[pp - tile_sz[0] * tile_sz[1]] + 20.0 * u[pp] -
    ...
    if (ijk_lm[4] == pw) {
        Du[IDX_L(i, j, (k_b - ijk_lm[4]))] =
            pre_factor_6_dz * (-u[IDX_L(i, j, (k_b - ijk_lm[4]) + 4)] +
    ...
    if (ijk_lm[5] == (sz[2] - 1 - pw)) {
        Du[IDX_L(i, j, (k_e - ijk_lm[4]))] =
            pre_factor_6_dz * (-u[IDX_L(i, j, (k_e - ijk_lm[4]) + 1)] +
