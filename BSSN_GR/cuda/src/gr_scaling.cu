...
int bssn_driver(MPI_Comm comm, unsigned int num_step, unsigned int warm_up,
    ...
    if (ts_mode == 0) {
        ...
        ts::ETS<DendroScalar, bssn::BSSNCtxGPU>* ets =
            new ts::ETS<DendroScalar, bssn::BSSNCtxGPU>(bssnCtx);
        ...
        while (ets->curr_time() < bssn::BSSN_RK_TIME_END) {
            ...
            {
              if(!rank_global)
                std::cout<<"[ETS] : Executing step :
            "<<ets->curr_step()<<"\tcurrent time :"<<ets->curr_time()<<"\t
            dt:"<<ets->ts_size()<<"\t"<<std::endl;
              // 03/23/22 : this works only because we use defult stream to for
            the evolution. Default stream is special and synchronous with all
            other streams. bssnCtx->device_to_host_async(s_gw);
            ...
            bssn::BSSN_LOAD_IMB_TOL,bssn::BSSN_SPLIT_FIX);
                    ...
                    bssn::BSSN_RK45_TIME_STEP_SIZE=bssn::BSSN_CFL_FACTOR*((bssn::BSSN_COMPD_MAX[0]-bssn::BSSN_COMPD_MIN[0])*((1u<<(m_uiMaxDepth-lmax))/((double)
            bssn::BSSN_ELE_ORDER))/((double)(1u<<(m_uiMaxDepth)))); ts::TSInfo
            ts_in = bssnCtx->get_ts_info(); ts_in._m_uiTh =
            bssn::BSSN_RK45_TIME_STEP_SIZE; bssnCtx->set_ts_info(ts_in);
                }
              }
