...
int main(int argc, char** argv) {
    ...
    // calling cuda init before mpi init needed for cuda aware mpi performacne.
    cuda_mpi_aware_init();
    MPI_Init(&argc, &argv);
    ...
    if (ts_mode == 1) {
        bssn::BSSNCtxGPU* bssnCtx = new bssn::BSSNCtxGPU(mesh);
        ...
        char fname[256];
        sprintf(fname, "bssnCtxGPU_%d.txt", npes);
        if (!rank) {
        ...
        while (ets->curr_time() < bssn::BSSN_RK_TIME_END) {
            ...
            if (bssn::BSSN_GW_EXTRACT_FREQ > 0 &&
                ...
                // 03/23/22 : this works only because we use defult stream to
                // for the evolution. Default stream is special and synchronous
