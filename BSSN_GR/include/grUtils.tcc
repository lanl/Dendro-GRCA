                ...
                for (unsigned int j = 0; j < (eleOrder + 1); j++)
                    for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                        ...
                        if ((nodeLookUp_CG >= nodeLocalBegin &&
                            ...
                            Point grid_pt(x, y, z);
                            const double d1 =
                                (grid_pt - bssn::BSSN_BH_LOC[0]).abs();
                            const double d2 =
                                (grid_pt - bssn::BSSN_BH_LOC[1]).abs();

                            if ((d1 < bssn::BSSN_BH1_CONSTRAINT_R ||
                                 d2 < bssn::BSSN_BH2_CONSTRAINT_R))
...
double extractConstraints(const ot::Mesh* mesh, const T** constraintVar,
    ...
    if (mesh->isActive()) {
        ...
        for (unsigned int index = 0; index < numConstraints; index++) {
            constraintMaskedL2[index] = bssn::computeConstraintL2Norm(
                mesh, constraintVar[index], maskVec, maskthreshoold);
            if (!rankActive) {
                std::cout << YLW << "\tConstraint "
                          << bssn::BSSN_CONSTRAINT_VAR_NAMES[index] << " L2 : ("
                          << constraintMaskedL2[index] << " )" << NRM
                          << std::endl;
            }
        }
