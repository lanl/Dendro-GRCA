...
Error prim_to_con(const ot::Mesh *pMesh, double **v, double **u) {
    ...
    for (unsigned int node = pMesh->getNodeLocalBegin();
        ...
        pos[0] = GRIDX_TO_X(allElements[ownerID].minX() +
                            ii_x * (sz / ((double)elementOrder)));
        pos[1] = GRIDY_TO_Y(allElements[ownerID].minY() +
                            jj_y * (sz / ((double)elementOrder)));
        pos[2] = GRIDZ_TO_Z(allElements[ownerID].minZ() +
                            kk_z * (sz / ((double)elementOrder)));
        ...
        double v_pp[FLUID_NUM_PRIM_VARS] = {
            v[PVAR::V_RHO][node], v[PVAR::V_VX][node], v[PVAR::V_VY][node],
            v[PVAR::V_VZ][node],  v[PVAR::V_P][node],  v[PVAR::V_U1][node],
            v[PVAR::V_U2][node],  v[PVAR::V_U3][node]};
        ...
        if (status != Error::FLUID_SOLVER_SUCCESS) failCount++;
    }

    if (failCount == 0)
        return Error::FLUID_SOLVER_SUCCESS;
    else {
        std::cout << "[Error]: primitive to conserved variable solver failed. "
                  << std::endl;
        print_stacktrace();
...
Error prim_to_con_pt(double v[], double u[], double gd[3][3], double sdetg) {
    ...
    if (Wsq < 1.0) {
        ...
        printf("prim_to_con_pt: metric: %f, %f, %f, %f, %f, %f\n", gd[0][0],
               gd[0][1], gd[0][2], gd[1][1], gd[1][2], gd[2][2]);
        print_stacktrace();
...
Error con_to_prim(const ot::Mesh *pMesh, double **u, double **v,
    ...
    for (unsigned int node = pMesh->getNodeLocalBegin();
        ...
        double u_pp[FLUID_NUM_EVOL_VARS] = {
            u[VAR::U_D][node], u[VAR::U_SX][node], u[VAR::U_SY][node],
            u[VAR::U_SZ][node], u[VAR::U_TAU][node]};
        double v_pp[FLUID_NUM_PRIM_VARS];
        ...
        // These variables are usually rescaled to be physical, but this
        // rescaling is a bit arbitrary and can cause other issues. So, under
        // those circumstances we add points that have been rescaled to a set.
        // These points are then interpolated using linear interpolation with up
        // to four nearest neighbors. If those points are unavailable, the code
        // leaves the points alone. Here's the basic overview for
        // implementation:
        ...
        //      - Check both x neighbors. If neither is in the bad point set,
        //      add them
        //        to a vector of averaging points.
        //      - Check both y neighbors. If neither is in the bad point set,
        //      add them
        //        to a vector of averaging points.
        //      - If the averaging vector is greater than zero, use compensated
        //        summation to average each the values of each point for each
        //        primitve variable. Add an option for rho to be averaged with
        //        log(rho), and remember to average the four-velocity instead of
        //        the three-velocity.
        // 3. Bad points on processor boundaries should be averaged after the
        // variables
        //    are unzipped so that they have access to an updated padding
        //    region.

        /*if(status!=Error::FLUID_SOLVER_SUCCESS)
        {
          std::cout<<"rank: "<<(pMesh->getMPIRank())<<"node : "<<node<<" con_to
        prim failed : "<<status<<std::endl; for(unsigned int
        var=0;var<FLUID_NUM_EVOL_VARS;var++) std::cout<<"cons var:
        "<<FLUID_EVOL_VAR_NAMES[var]<<" value: "<<u_pp[var]<<std::endl;
        }*/
    ...
    if (failCount == 0)
        ...
        std::cout << "[Error]: conserved to primitive variable solver failed. "
                  << std::endl;
        print_stacktrace();
...
Error con_to_prim_pt(double *dupt, double *vpt, double gd[3][3],
                     double gu[3][3], double sdetg, double pos[]) {
    ...
    if (upt[VAR::U_D] < FLUID_VACUUM) {
        ...
        double Ssq_max =
            (2.0 * upt[VAR::U_D] + upt[VAR::U_TAU]) * upt[VAR::U_TAU];
        if (Ssq_max < Ssq) {
    ...
    if (rc < 0) return Error::FLUID_CONS_TO_PRIM_FAILED;
...
Error Vvec3_to_Vvec4(double *v3, double *v4, double gd[3][3], double alpha,
                     double Beta[3]) {
    const double vSq = square_vector(v3, gd);
    if (vSq > 1.0) return Error::FLUID_V_SQUARE_GT_1;
...
Error Vvec4_to_Vvec3(double *v4, double *v3, double gd[3][3], double alpha,
                     double Beta[3]) {
    double Betad[3];
    ...
    double inv_W           = (alpha - bsq * inv_alpha) /
                   (bu + sqrt(bu * bu + (alpha * alpha - bsq) * (1.0 + usq)));
...
void c2p_work_vars(double *fpar, double *u, double *v, double gd[3][3],
    ...
    if (vsq >= 1.0) {
        printf(
            "  c2p_work_vars:  The velocity is superluminal.  To correct \n");
...
// mhd_solver_4 {{{
// This is the most robust and accurate primitive solver without a magnetic
// field.
int mhd_solver_4(double *u, double *v, double gd[3][3], double gu[3][3],
    ...
    // CASE 2
        ...
        if (ltrace) {
            ...
            printf(
                "  mhd_solver_4:  violation.  In particular, Ssq/a^2 + "
                "D^2/a^2\n");
            ...
            printf("     \n");
            printf(
                "  mhd_solver_4:  We will rescale S_i and Ssq in hopes this "
                "\n");
        ...
        */
        ...
        if (t2 < 0.0) {
            printf("mhd_solver_4: t2 < 0, i.e. t2=%25.20e\n", t2);
        }
        ...
        if (qg_2 < qf_2) {
            ...
            if (ltrace) {
                //{{{
                printf(
                    "  mhd_solver_4:  Even after rescaling, we still have a "
                    "\n");
                printf(
                    "  mhd_solver_4:  problem with the relative values of the "
                    "\n");
                printf(
                    "  mhd_solver_4:  bounds qg_2 and qf_2.  In particular, we "
                    "n");
                printf(
                    "  mhd_solver_4:  should have that  qg_2 < qf_2. Instead, "
                    "we\n");
        ...
        // Because we made a rescaling of the components of S and of Ssq, we
        // need to save these.
        u[VAR::U_SX] = Sd[0];
        u[VAR::U_SY] = Sd[1];
        u[VAR::U_SZ] = Sd[2];
        ...
        if (beta_sq + (D / a) * (D / a) >= 1.0) {
            if (ltrace) {
                printf(
                    "  mhd_solver_4:  Even after rescaling S_i the physical "
                    "\n");
                printf(
                    "  mhd_solver_4:  inequality that should be satisfied is "
                    "not\n");
            ...
            if (ltrace) {
                printf(
                    "  mhd_solver_4:  After what appears to be a successful "
                    "\n");
    ...
    // CASE 3
        ...
        printf(
            "  mhd_solver_4:  possibility as I didn't think it ever arose. \n");
        printf("  mhd_solver_4:  I guess it does after all. \n");
        /*
            printf("  mhd_solver_4:  D/a is so small as to be lost in
        roundoff.\n"); printf("  mhd_solver_4:  We will adjust Ssq in hopes this
        \n"); printf("  mhd_solver_4:  resolves the problem. \n");

            //Instead of a true (multiplicative) rescaling, in this case, we
        have
            ...
            //  FIXME:  Actually, this could be a real problem.  If Sd is
        already negative, we could be making the magnitude, Ssq, in fact larger.
        this is definitely a potential problem and should be addressed ... for (
        j=0; j<3; j++ ) { Sd[j] -= 1.e-15 * a ; }

            // Recalculate Ssq.
            Ssq = square_form(Sd, gu);

            // Now we have to recalculate the roots of f and g with this new
        Ssq. beta_sq = Ssq/(a*a); t2 = 1.0 - (gamma-1.0)*beta_sq/(t1*t1) ; if
        (t2 < 0.0) { printf("mhd_solver_4: t2 < 0, i.e. t2=%25.20e\n",t2); }
            qf_2 = t1*( 1.0 + sqrt(t2)) ; // new root of f; upper bound on q
            qg_2 = sqrt(beta_sq) ;        // new root of g; lower bound on q

            // Check that these new roots are indeed good upper and lower
        bounds. if ( qg_2 < qf_2 )
            {
              ql = qg_2 ;
              qh = qf_2 ;
        ...
        \n"); printf("  mhd_solver_4:  bounds  qg_2  and  qf_2.  In
        particular,\n"); printf("  mhd_solver_4:  we should have  qg_2 < qf_2.
        Instead, we \n"); printf("  mhd_solver_4:  have \n"); printf("
        mhd_solver_4:      qg_2 = %25.20e \n", qg_2 ); printf("  mhd_solver_4:
        qf_2 = %25.20e \n", qf_2 );
            }

            // Because we made a change to the S components and Ssq, we need to
        save
            // them.
            u[U_SX] = Sd[0] ;
            u[U_SY] = Sd[1] ;
        ...
        \n"); printf("  mhd_solver_4:      fabs( (beta_sq + (D/a)*(D/a)) -
    ...
    // CASE 4
        ...
        // {{{
        // This else is effectively a garbage dump for all the other
        // possibilities not delineated in the if/else above.  It is a sort of
        // "none of the above". As a result, if we do get here, we essentially
        // don't know why...
    ...
    if (f_at_ql < 0.0 || f_at_qh > 0.0) {
        if (ltrace) {
            //{{{
            printf(
                "  mhd_solver_4:  There is a problem with the bracketing of "
                "\n");
            ...
            printf("  mhd_solver_4:    f_at_qh = %25.20e should be < 0.0\n",
                   f_at_qh);
            printf(
                "  mhd_solver_4:  We are sort of at a loss at this point.  We "
                "\n");
            printf(
                "  mhd_solver_4: will go on, but without much hope of "
                "success.\n");
            //}}}
        }
    }
    ...
    if (f_at_ql > 0.0 && f_at_qh > 0.0) {
        if (ltrace) {
            ...
            printf(
                "  mhd_solver_4:  We will try to shift the current value \n");
            printf(
                "  mhd_solver_4:  of qh (which is %25.20e)up by 6.e-16,i.e.\n",
                qh);
        }
        ...
        if (ltrace) {
            ...
            printf(
                "  mhd_solver_4:  value of f (which should now be negative) "
                "\n");
            ...
            printf(
                "  mhd_solver_4:    new f_at_qh = %25.20e (should be neg) \n",
                f_at_qh_v2);
        }
    }
    ...
    */
    ...
    if (v[PVAR::V_RHO] < 0.0 || v[PVAR::V_P] < 0.0 || vsq < 0.0 || vsq > 1.0) {
        if (ltrace) {
            // {{{
            printf(
                " mhd_solver_4:  Problematic values after the primitive. \n");
...
int SolvePrimVarsIsentropic(double *qa, double *upt, double *fpar) {
    ...
    if (rc < 0) {
        if (FLUID_CONTOPRIMWARN == 1) {
            fprintf(stdout,
                    "SolvePrimVarsIsentropic: Could not bracket the root for "
                    "hW^2\n");
...
int func4_p(double *f, double q, double *fpar) {
    ...
    *f          = -qoff * qoff + gamma * gamma / 4.0 - (gamma - 1.0) * betasq -
         (gamma - 1.0) * D / a * sqrt(dis);
...
void MathDump(double *fpar) {
    ...
    fprintf(stdout, "COORDS: (x,y,z) = (%g, %g, %g)\n", fpar[FP_COORD_X],
            fpar[FP_COORD_Y], fpar[FP_COORD_Z]);
...
int func_p_d_isentropic(double *f, double *df, double rho0, double *fpar) {
    ...
    *f  = (h * h * W * W) * (W * W - 1.0) - Ssq;
    ...
    t52 = 1.0 / t51;
    t53 = t10 * t10;
    *df = (2.0 * t7 * t8 * t11 * t17 - 2.0 * t22 * t24) * (t8 * t11 - 0.1E1) -
          2.0 * (t22 * t11) * t8 * t24;
