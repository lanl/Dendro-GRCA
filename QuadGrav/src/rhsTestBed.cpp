...
#pragma omp parallel for default(none) shared(blkList,pt_min,pt_max,varUnzipIn,bssn::BSSN_COMPD_MAX,bssn::BSSN_COMPD_MIN,bssn::BSSN_OCTREE_MAX,bssn::BSSN_OCTREE_MIN) schedule(dynamic)
        ...
        for(unsigned int k=0;k<sz[2];k++)
        {
            ...
            for(unsigned int j=0;j<sz[1];j++)
            {
                ...
                for(unsigned int i=0;i<sz[0];i++)
                {
                    ...
                    bssn::fake_initial_data(X_TO_GRIDX(x),Y_TO_GRIDY(y),Z_TO_GRIDZ(z),varVal);

                    for(unsigned int v=0;v<bssn::BSSN_NUM_VARS;v++)
                        varUnzipIn[v][offset+k*sz[0]*sz[1]+j*sz[0]+i]=varVal[v];
    ...
    t1=Time::now();
#pragma omp parallel for default(none) shared(blkList,pt_min,pt_max,varUnzipIn,varUnzipOutCPU0,bssn::BSSN_COMPD_MAX,bssn::BSSN_COMPD_MIN,bssn::BSSN_OCTREE_MAX,bssn::BSSN_OCTREE_MIN) schedule(dynamic)
    for(unsigned int blk=0;blk<blkList.size();blk++) {
...
#pragma omp parallel for default(none) shared(blkList,pt_min,pt_max,varUnzipIn,varUnzipOutCPU1,bssn::BSSN_COMPD_MAX,bssn::BSSN_COMPD_MIN,bssn::BSSN_OCTREE_MAX,bssn::BSSN_OCTREE_MIN) schedule(dynamic)
    for(unsigned int blk=0;blk<blkList.size();blk++) {
