...
void cuda::profile::printOutput(const ot::Mesh* pMesh) {
    ...
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator) <<"+cuda_overall(s)";
    ...
    computeOverallStats(&t_stat, t_stat_g, comm);
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator) <<"  -H2D(s)";
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[0];
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[1];
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[2]<<std::endl;
    ...
    computeOverallStats(&t_stat, t_stat_g, comm);
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator) <<"  -D2H(s)";
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[0];
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[1];
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[2]<<std::endl;
    ...
    computeOverallStats(&t_stat, t_stat_g, comm);
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator) <<"  -deriv(malloc)(s)";
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[0];
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[1];
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[2]<<std::endl;
   ...
   /* t_stat=t_derivs.seconds;
    computeOverallStats(&t_stat, t_stat_g, comm);
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator) <<"  -deriv(compute)(s)";
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[0];
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[1];
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[2]<<std::endl;

    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator) <<"  -deriv(compute)(flops)";
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<(((unzipInternal_g*NUM_DERIV_OPS_PP)+totalBloks*NUM_DERIV_OPS_BLOCK)/t_stat_g[0]);
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<(((unzipInternal_g*NUM_DERIV_OPS_PP)+totalBloks*NUM_DERIV_OPS_BLOCK)/t_stat_g[1]);
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<(((unzipInternal_g*NUM_DERIV_OPS_PP)+totalBloks*NUM_DERIV_OPS_BLOCK)/t_stat_g[2])<<std::endl;*/

    t_stat=t_rhs_cpu.seconds;
    computeOverallStats(&t_stat, t_stat_g, comm);
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator) <<"  -rhs_cpu(compute)(s)";
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[0];
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[1];
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[2]<<std::endl;
    ...
    computeOverallStats(&t_stat, t_stat_g, comm);
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator) <<"  -rhs_gpu(compute)(s)";
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[0];
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[1];
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[2]<<std::endl;
    ...
    computeOverallStats(&t_stat, t_stat_g, comm);
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator) <<"  -rhs_total(compute)(s)";
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[0];
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[1];
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<t_stat_g[2]<<std::endl;

    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator) <<"  -overall(flops)";
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<(((unzipInternal_g*(NUM_DERIV_OPS_PP+NUM_RHS_OPS_PP))+totalBloks*NUM_DERIV_OPS_BLOCK)/t_stat_g[0]);
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<(((unzipInternal_g*(NUM_DERIV_OPS_PP+NUM_RHS_OPS_PP))+totalBloks*NUM_DERIV_OPS_BLOCK)/t_stat_g[1]);
    if(!rank)std::cout << std::left << std::setw(nameWidth) << std::setfill(separator)<<(((unzipInternal_g*(NUM_DERIV_OPS_PP+NUM_RHS_OPS_PP))+totalBloks*NUM_DERIV_OPS_BLOCK)/t_stat_g[2])<<std::endl;
